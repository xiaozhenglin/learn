...
public class WeChatRefundResponse implements Serializable {
    private static final long serialVersionUID = -8761616711607415863L;
    ...
    private Integer refund_fee;//退款金额	是	Int	100	退款总金额,单位为分,可以做部分退款
    private Integer settlement_refund_fee;//应结退款金额	否	Int	100	去掉非充值代金券退款金额后的退款金额，退款金额=申请退款金额-非充值代金券退款金额，退款金额<=申请退款金额
    private Integer total_fee;//标价金额	是	Int	100	订单总金额，单位为分，只能为整数，详见支付金额
    ...
    private Integer cash_fee;//现金支付金额	是	Int	100	现金支付金额，单位为分，只能为整数，详见支付金额
    private String cash_fee_type;//现金支付币种	否	String(16)	CNY	货币类型，符合ISO 4217标准的三位字母代码，默认人民币：CNY，其他值列表详见货币类型
    private Integer cash_refund_fee;//现金退款金额 否	Int	100	现金退款金额，单位为分，只能为整数，详见支付金额
    ...
    private Integer coupon_refund_fee;//代金券退款总金额否	Int	100	代金券退款金额<=退款金额，退款金额-代金券或立减优惠退款金额为现金，说明详见代金券或立减优惠
    private Integer coupon_refund_fee_$n;//单个代金券退款金额	否	Int	100	代金券退款金额<=退款金额，退款金额-代金券或立减优惠退款金额为现金，说明详见代金券或立减优惠
    private Integer coupon_refund_count;//退款代金券使用数量	否	Int	1	退款代金券使用数量
